class GameScene extends egret.Sprite {
	...
	starArr : Array<Block> = [];
	resetSlider:egret.Timer;
	resetNum:number = 0;
	score:number = 0;
	showScore:egret.TextField;
	...
	private addToStageHd(e: egret.Event) {
		this.addObjectHd();
		this.addEventHd();
	}
	...
	//开始游戏
	private init(e:egret.TouchEvent){
		this.bullet.canLunch = true;
		var angle:number = Math.atan2(e.stageY-this.slider.y,e.stageX-this.slider.x);
	...
	private dosomething(i:number,j:number){
		this.resetNum++;
	...
	private collisiionDo(e1:any,e2:Bullet){
	...
	private collisionSlider(){
		Collision.block(this.slider,this.bullet,'slider');
		var divergeAngle:number,over_y:number,over_x:number;
		if(Collision.collisionSide == 'Top' || Collision.collisionSide =='Bottom'||
			Collision.collisionSide == 'Right' || Collision.collisionSide == 'Left'){
	...
	private touchMoveHd(e:egret.TouchEvent){
	...
	private createBlock() {
		var h_num:number = GameFactory.block_Vnum;//5行 6列
		var v_num:number = GameFactory.block_Hnum;
		var color:number;
	...
	private createWall() {
		var wallLeft: Block = new Block(GameFactory.BLOCK_BLACK, GameFactory.wallWidth, GameFactory.wallHeight);
		var wallRight: Block = new Block(GameFactory.BLOCK_BLACK, GameFactory.wallWidth, GameFactory.wallHeight);
		var wallTop: Block = new Block(GameFactory.BLOCK_BLACK, GameFactory.wallTopWidth, GameFactory.wallTopHeight);
	...
	private createStar(name,color:number,sha):Block{
		var stars:Block = new Block(color,GameFactory.blockHeight,GameFactory.blockHeight);
	...
	//鉴别某个砖块是否需要碰撞检测
	private sarchColled(x:number,y:number){
		//判断方向 → ↓ ← ↑
