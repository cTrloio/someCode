...
class Main extends egret.DisplayObjectContainer {
    private boxArr:any=new Array();
    private bugArr:Array<BugWings>=[];
    ...
    private bugWing1:BugWings;
    private bugWing2:BugWings;
    private bugWing3:BugWings;
    ...
    private enterFrame(e:egret.Event) {
        ...
        for(var i:number=0;i<this.boxArr.length;i++)
        {
            for(var j:number=0;j<this.bugArr.length;j++){
    ...
    //蚊子人物碰撞
    public charaHitBug(bug:BugWings) {
    ...
    //创建随机数
    private createRandomNum(mon:BugWings) {
    ...
    private gameOver() {
        ...
        {
            var gameObjectBitMap:DisplayBackground=new DisplayBackground();
            this.addChild(gameObjectBitMap);
            gameObjectBitMap.x=150;
            gameObjectBitMap.y=150;

            var test:egret.TextField = new egret.TextField();   //自适应宽高
            test.text = "游戏结束！";
            ...
            test.textFlow = <Array<egret.ITextElement>>[
                {
