...
class OneScene extends egret.Sprite{
    ...
    // 4(n-1)___4n
    trueStr:string='出生入死后羿射日乐在其中琴棋书画' +
                    '理直气壮万紫千红坐井观天喜从天降山盟海誓趁热打铁' +
                    '三生有幸德高望重穿针引线五雷轰顶摇摇欲坠才' +
                    '高八斗战战兢兢重男轻女进退两难';
    // 扰乱字符
    falseStr:string='十而立交加寿比南崩地裂一笑千金牛郎织女'+
    ...
    mm:MusicManger;
    tailStrArr:Array<string>=[];  //乱序其他字符
    intdArr:Array<string>=[];     //正序成语解释
    sponArr:Array<string>=[];     //正序成语出处
    testArr:Array<string>=[];     // 每一关的所有字符=(正序四字成语+乱序其他字符)
    testArr1:Array<string>=[];    // 正序四字成语
    testArr2:Array<string>=[];    // 乱序其他字符
    collArr:Array<cyTest>= [];    // 容纳成语的容器数组
    teArr:Array<TextBox>=[];      // 所有字数组

    static maxSceneNum:number=1;    //记录过了多少关 最多现在19关
    bg:Background;
    backBtn:Background;
    mOn:MusicSetBtn;
    promPanel:promptPic;
    ...
    private addObjectHd(){
        ...
        for(var j=0;j<this.testArr.length;j++){
            var tb:TextBox=new TextBox('teBox');
    ...
    private tapHd(e:egret.TouchEvent){
        this.mm.playclickM();
        e.target.clickNum++;
        if(e.target.clickNum%2==1){  //把某个字点上去
            for(var i=0;i<this.collArr.length;i++){ //循环判断容器里是否有对象存在
                if(!this.collArr[i].havChild){ //如果可以容纳 则设置其位置 并将此容器 是否有对象存在 设为 true
