...
class GameScene extends egret.Sprite {
    ...
    bulletArr: Array<Bullet> = [];
    boosBulletArr: Array<Bullet> = [];
    enemyArr: Array<Enemy> = [];
    starArr: Array<Word> = [];

    fireTimer: egret.Timer;
    boosFireTimer: egret.Timer;
    enemyTimer: egret.Timer;
    boosTimer: egret.Timer;
    ...
    private addObjectd() {
        ...
        var firebut:fireBtn = new fireBtn();
        ...
        var sqp:egret.Sprite = new egret.Sprite();
        sqp.graphics.clear();
    ...
    private createEnemy(who: string) {
        var enemy: Enemy = new Enemy(who, this);
        this.addChild(enemy);
        this.enemyArr.push(enemy);
        this.setXY(enemy);
    }
    private setXY(enemy: Enemy) {
        var randomNum = GameData.getRandomNum();
        var num: number = Math.floor(randomNum * 4);
        var angle: number = 0;
        switch (num) {
            case 0:
    ...
    private createBullet(type: string, num: number, obj: any, arr: Array<Bullet>) {
        for (var i = 0; i < num; i++) {
            var bull: Bullet = new Bullet(type);
            this.addChild(bull);
            arr.push(bull);
            if (obj == this.chara) {
                bull.x = obj.x;
    ...
    private createBoos(type: string) {
    ...
    private checkBullet(obj: Array<Bullet>) {
        if (obj.length != 0) {
    ...
    private gameOver() {
        ...
        var maxSocre: number = Number(GameData.MAXSCORE);
