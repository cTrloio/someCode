class SceneOne extends egret.Sprite {
    public constructor() {
        super();
        this.addEventListener(egret.Event.ADDED_TO_STAGE, this.addStageHd, this);
    }

    playNameBtn:eui.Button;
    playPanel:eui.Panel;
    playerLabel:eui.Label;
    playerTxt:egret.TextField;
    ...
    public static playName:string = '';
    public static playMoney:number = 0;
    chara:Character;
    bg:Background;
    timer:egret.Timer;
    //box数组
    ...
    btnFive:CtrlButton;
    //金币
    moneyIcoSmall:CtrlButton;
    //时间条
    progressBar:eui.ProgressBar;
    speed:number = 700;
    //全局计时器
    curnTimer:egret.Timer;
    //得分
    textScore:egret.TextField;
    //分数
    scoreNum:number = 0;
    //人物类型
    charaType:string;
    //music
    sound:egret.Sound;//计分音效
    sound1:egret.Sound;//bgm音效
    sound2:egret.Sound;//金币音效
    soundChannel:egret.SoundChannel;//bgm
    soundChannelTwo:egret.SoundChannel;//score
    soundChannelThree:egret.SoundChannel;//金币
    //设置按钮
    musicOn:eui.Button;
    setBtn:eui.Button;
    //创建金币条件
    clickCount:number = 0;
    //掉落金币
    moneyI:CtrlButton = null;
    //是否碰撞 游戏结束条件
    hitObj:boolean = false;
    //显示金币
    textMoney:egret.TextField;
    //碰到金币数量
    hitMoneyIcoNum:number = 0;
    ...
    private btnThings(obj:CtrlButton) {
        this.clickCount++;
        obj.alpha = 1;
        var tw1:egret.Tween = egret.Tween.get(obj, {loop: false});
        tw1.to({alpha: 0}, 500);
        this.doSomething();
    }

    ctrlBoxType:number = 0;
    ...
    private gameOverPanel() {
        ...
        //得分显示
        var money2:egret.TextField = new egret.TextField();
    ...
    private onButtonClickAgian(e:egret.TouchEvent) {
    ...
    private onButtonClickOver(e:egret.TouchEvent) {
        this.removeChild(this.panel);
        //停止一切
        this.Over();
        this.replayBtn.removeEventListener(egret.TouchEvent.TOUCH_TAP, this.onButtonClickAgian, this);
